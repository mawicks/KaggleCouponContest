...
accumulators = (
    ...
    naive_bayes_wrapper.SimpleNBWrapper(user_history, coupon, 'QUANTIZED_AGE', 'QUANTIZED_DISCOUNT_PRICE'),
    naive_bayes_wrapper.SimpleNBWrapper(user_history, coupon, 'QUANTIZED_AGE', 'QUANTIZED_CATALOG_PRICE'),
    ...
    naive_bayes_wrapper.SimpleNBWrapper(user_history, coupon, 'SEX_ID', 'QUANTIZED_DISCOUNT_PRICE'),
    naive_bayes_wrapper.SimpleNBWrapper(user_history, coupon, 'SEX_ID', 'QUANTIZED_CATALOG_PRICE'),
...
#            'gender_discount_price',
#            'gender_catalog_price',
...
#            accumulators[39].score(coupon_hash, user_hash, date),
            ...
            accumulators[41].score(coupon_hash, user_hash, date),
            accumulators[42].score(coupon_hash, user_hash, date),
            accumulators[43].score(coupon_hash, user_hash, date),

            accumulators[44].score(coupon_hash, user_hash, date),
        )
...
accumulators[40].dump(10)
accumulators[41].dump(10)
accumulators[42].dump(10)
accumulators[43].dump(10)
accumulators[44].dump(10)
...
for i in range(n_negative):
    purchasing_user_hash,purchase_week_index = random_list_element(random_state, purchase_list)
...
positive_train_weights = [ -math.log2(p + 1e-6) for p in regressor.oob_prediction_[:positive_train_size] ]

logger.info('Min score for a purchase: {0}'.format(min(regressor.oob_prediction_[:positive_train_size])))
logger.info('Max score for a non-purchase: {0}'.format(max(regressor.oob_prediction_[positive_train_size:])))
...
for _ in range(7):
    ...
    beta = 0.5  # beta=1.0 is reasonably fast; beta=0.0 is too slow.  Useful values are closer to 1.
    density_shaper = shaper.Shaper(random_state, desired_density = lambda x: (1-x)**beta)
    # density_shaper = shaper.QuotaShaper(desired_density = lambda x: 1)
    ...
    for i in range(n_negative):
        ...
        f = features(user_history[purchasing_user_hash], random_coupon, purchase_week_start)
        score = regressor.predict([f])[0]
        negative_features.append(f)
        negative_users.append(purchasing_user_hash)
        negative_coupons.append(random_coupon_hash)
        negative_weights.append( -math.log2(1.0 - score + 1e-6))
        if i % 100 == 0:
    ...
    train_features = positive_features[:positive_train_size] + negative_features[:negative_train_size]
    train_outcomes = positive_train_size * [1] + negative_train_size * [0]
    train_weights = positive_train_weights + negative_weights[:negative_train_size]
    ...
    logger.info('Training {0}: {1}'.format(name, regressor))
    regressor.fit(train_features, train_outcomes, sample_weight=train_weights)
    positive_train_weights = [ -math.log2(p + 1e-6) for p in regressor.oob_prediction_[0:positive_train_size] ]
